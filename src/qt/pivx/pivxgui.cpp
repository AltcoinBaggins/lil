...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Cortez client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Cortez"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Cortez"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
